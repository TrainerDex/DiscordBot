...
class QuickStart(commands.Cog):
    ...
    async def quickstart(self, ctx: commands.Context) -> None:
        ...
        try:
            mystic_role = min([x for x in ctx.guild.roles if _('Mystic').lower() in x.name.lower()])
        except ValueError:
            mystic_role = None
        if mystic_role:
            await getattr(self.config.guild(ctx.guild), 'mystic_role').set(mystic_role.id)
            await ctx.send(_("`{key}` set to {value}").format(key='mystic_role', value=mystic_role.mention))
        ...
        if valor_role:
            ...
            await ctx.send(_("`{key}` set to {value}").format(key='valor_role', value=valor_role.mention))
        
        try:
            instinct_role = min([x for x in ctx.guild.roles if _('Instinct').lower() in x.name.lower()])
        except ValueError:
            instinct_role = None
        if instinct_role:
            await getattr(self.config.guild(ctx.guild), 'instinct_role').set(instinct_role.id)
            await ctx.send(_("`{key}` set to {value}").format(key='instinct_role', value=instinct_role.mention))
        
        await ctx.send(_("Looking for TL40 role..."))
    
        try:
            tl40_role = min([x for x in ctx.guild.roles if _('Level 40').lower() in x.name.lower()])
        except ValueError:
            tl40_role = None
        if tl40_role:
            await getattr(self.config.guild(ctx.guild), 'tl40_role').set(tl40_role.id)
            await ctx.send(_("`{key}` set to {value}").format(key='tl40_role', value=tl40_role.mention))
